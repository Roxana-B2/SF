...
class Track:
    ...
    def __init__(self, meas, id):
        ...
        # set up covariance of position estimation with error
        M_rot = meas.sensor.sens_to_veh[0:3, 0:3]
        P_pos = M_rot * meas.R * np.transpose(M_rot)
        
        # set up covariance of velocity estimation with error
...
class Trackmanagement:
    ...
    def manage_tracks(self, unassigned_tracks, unassigned_meas, meas_list):  
        ...
        for tr in self.track_list:
            # check if the score is below the predefined threshold for discarding the track as possibly irrelevant noise
            # also check if either the px or py value of the covariance P exceeds the max value (not useful anymore)
            # in order not to delete initialized or tentative tracks right away
