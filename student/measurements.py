...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # avoid division by zero, ignore negative vals
        if pos_sens[0] > 0:
            alpha = np.arctan(pos_sens[1] / pos_sens[0])
            # actual value check
            if self.fov[0] <= alpha and alpha <= self.fov[1]:
                visible = True
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # avoid division by 0
            if pos_sens[0] == 0:
                raise NameError('Invalid x value, we compute no Jacobian matrix for it')
            else:
